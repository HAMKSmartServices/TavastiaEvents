...
"""
    Tavastia Events
    Lisätty get_random_string ja PointField import
    PointField pluginista django-extra-fields
"""
...
"""
    Tavastia Events
    Alkuperäinen id generointi käyttää organisaation id:tä luontiin
"""
...
"""
    Tavastia Events
    poistettu funktio get_authenticated_data_source_and_publisher, korvattu
    get_data_source_and_publisher funktiolla, jolla palautetaan tyhjät arvot
"""
"""
...
"""
def get_data_source_and_publisher(request):
    publisher = None
    data_source = None
    return data_source, publisher
...
class LinkedEventsSerializer(TranslatedModelSerializer, MPTTModelSerializer):
    ...
    def __init__(self, instance=None, files=None,
        ...
        """
            Tavastia Events
            self.user tarkistus poistettu, ei käyttäjiä
            self.data_source, self.publisher haetaan dummy funktiosta, get_data_source_and_publisher
            alkuperäinen funktio poistettu
            if not self.publisher poistettu
        """
        ...
        self.data_source, self.publisher = get_data_source_and_publisher(request)
    ...
    """
        AvoinHäme
        funktiot validate_data_source ja validate_publisher käyttöoikeustarkistukset poistettu
    """
    ...
    def create(self, validated_data):
        ...
        """
            Tavastia Events
            validated_data['created_by'] ja validated_data['last_modified_by'] korvattu tyhjillä arvoilla
        """
    ...
    """
        Tavastia Events
        Käyttöoikeustarkistukset poistettu
        last_modified_by korvattu tyhjällä arvolla
    """
...
class PlaceSerializer(LinkedEventsSerializer, GeoModelSerializer):
    ...
    class Meta:
        model = Place
        exclude = ('n_events_changed', 'origin_id')
...
"""
    Tavastia Events
    Alkuperäiset paikkaan liittyvät luokat
"""
"""
...
"""
    Tavastia Events
    Serializer POST metodille, GET metodi käyttää alkuperäistä serializeria
"""
class PlacePostSerializer(TranslatedModelSerializer):
    view_name = 'place-detail'
    ...
    def validate(self, data):
        ...
        fields_to_clean = [
            'name', 'name_fi', 'name_sv', 'name_en',
            'description', 'description_fi', 'description_sv', 'description_en',
            'street_address', 'street_address_fi', 'street_address_sv', 'street_address_en',
            'address_locality', 'address_locality_fi', 'address_locality_sv', 'address_locality_en',
            'telephone', 'telephone_fi', 'telephone_sv', 'telephone_en',
            'post_office_box_num',
            'custom_data'
        ]

        for k, v in data.items():
            if k in fields_to_clean:
                if isinstance(v, str) and any(c in v for c in '<>&'):
                    data[k] = bleach.clean(v, settings.BLEACH_CLEAN, strip=True)
                    data[k] = data[k].replace('&amp;', '&')
        
        if 'custom_data' in data:
            for key, value in data['custom_data'].items():
                if isinstance(value, str) and any(c in value for c in '<>&'):
                    data['custom_data'][key] = bleach.clean(value, settings.BLEACH_CLEAN, strip=True)
                    data['custom_data'][key] = data['custom_data'][key].replace('&amp;', '&')
        ...
        if 'name_en' not in data:
            ...
            #if data['name_en'] != None and data['name_en'] != "":
...
"""
    Tavastia Events
    PlaceModelViewSet
    Tarkistaa metodin GET/POST ja käyttää sen mukaista serializeria
"""
...
class ImageViewSet(viewsets.ModelViewSet):
    ...
    """
        Tavastia Events
        Sallitut metodit rajattu get, post ja head
    """
    http_method_names = ['get', 'post', 'head']
    ...
    def perform_destroy(self, instance):
        ...
        data_source, organization = get_data_source_and_publisher(self.request)
...
"""
    Tavastia Events
    Serializerit POST, PUT ja GET metodeille
    Erikseen PIN-koodin vuoksi
"""

class EventPostSerializer(LinkedEventsSerializer, GeoModelAPIView):
    ...
    event_status = EnumChoiceField(Event.STATUSES, required=False, allow_null=True)
    publication_status = EnumChoiceField(PUBLICATION_STATUSES, required=False, allow_null=True)
    ...
    publisher = serializers.PrimaryKeyRelatedField(queryset=Organization.objects.all(), required=False, allow_null=True)
    ...
    #info_url = serializers.URLField(allow_null=True, required=False, allow_blank=True)
    created_time = DateTimeField(default_timezone=pytz.UTC, required=False, allow_null=True)
    last_modified_time = DateTimeField(default_timezone=pytz.UTC, required=False, allow_null=True)
    ...
    """
        validate_id poistettu alkuperäinen, korvattu TRP-merkinnällä
    """
    def validate_id(self, value):
        if value:
            #id_data_source_prefix = value.split(':', 1)[0]
            id_data_source_prefix = 'TRP'
            # if not id_data_source_prefix == self.data_source.id:
            if not id_data_source_prefix == 'TRP':
                # the event might be from another data source by the same organization, and we are only editing it
                if self.instance:
    ...
    def validate(self, data):
        ...
        desc_to_clean = [
            'description', 'description_fi', 'description_en', 'description_sv'
          ]

        fields_to_clean = [
            'name', 'name_fi', 'name_sv', 'name_en',
            'short_description', 'short_description_fi', 'short_description_sv', 'short_description_en',
            'location_extra_info',
            'provider', 'provider_fi', 'provider_sv', 'provider_en',
            'pin'
            'info_url', 'info_url_fi', 'info_url_sv', 'info_url_en'
        ]
        ...
        #        for key, value in data['custom_data'].items():
        #            if isinstance(value, str) and any(c in value for c in '<>&'):
        #                #print(data['custom_data'], "fulldata unclean")
        #                #print(data['custom_data'][key],"unclean")
        #                data['custom_data'][key] = bleach.clean(value, settings.BLEACH_CLEAN, strip=True)
        #                data['custom_data'][key] = data['custom_data'][key].replace('&amp;', '&')
        #                #print(data['custom_data'][key],"cleaned")
        #                #print(data['custom_data'], "fulldata clean")
        ...
        """
            Tavastia Events
            pin default arvoksi 0000
            description asetetaan short_description mukaan, jos tyhjä
        """
        errors = {}
        if 'accessible' not in data:
            errors['accessible'] = (_('Accessibility information is needed'))
        ...
        if 'in_language' not in data:
            ...
            data['in_language'].append(Language.objects.get(id='fi'))
        #    validated_data['in_language'] = validated_data['in_language'].append(Language.objects.get(id='fi'))
        ...
        # helsinki short_descr pituus 160 merkkiä
        ...
        for field in self.fields_needed_to_publish:
            if field in self.translated_fields:
                for lang in languages:
                    ...
                    if field == 'short_description' and len(data.get(field_lang, [])) > 140:
        ...
        fields_to_clean_offer = [ 'description', 'description_fi', 'description_en', 'description_sv'
                            'price', 'price_fi', 'price_sv', 'price_en',
                            'info_url', 'info_url_fi', 'info_url_sv', 'info_url_en'
          ]
        ...
        for offer in data.get('offers', []):
            for k, v in offer.items():
                if k in fields_to_clean_offer:
                    if isinstance(v, str) and any(c in v for c in '<>&'):
                        offer[k] = bleach.clean(v, settings.BLEACH_CLEAN, strip=True)
                        offer[k] = offer[k].replace('&amp;', '&')
    ...
    def create(self, validated_data):
        ...
        """
            Tavastia Events
            Asetetaan tapahtumien yleiskuva oletuksena
        """
        if 'image' not in validated_data:
            try:
                validated_data['image'] = Image.objects.get(id=27) # 27
    ...
    def to_representation(self, obj):
        ret = super(EventPostSerializer, self).to_representation(obj)
...
"""
    Tavastia Events
    Tapahtuman päivitys ja poistaminen
"""

class EventPutDeleteSerializer(LinkedEventsSerializer, GeoModelAPIView):
    id = serializers.CharField(required=False)
    location = JSONLDRelatedField(serializer=PlaceSerializer, required=False,
                                  view_name='place-detail', queryset=Place.objects.all())
    # provider = OrganizationSerializer(hide_ld_context=True)
    ...
    """
        pin vaaditaan, mutta ei voida vaihtaa
    """
    ...
    #fields_needed_to_publish = ('keywords', 'location', 'start_time', 'short_description', 'description')
    fields_needed_to_publish = ('keywords', 'location', 'start_time', 'short_description')
    #fields_needed_to_publish = ('short_description')

    def __init__(self, *args, skip_empties=False, **kwargs):
        super(EventPutDeleteSerializer, self).__init__(*args, **kwargs)
        # The following can be used when serializing when
        # testing and debugging.
        self.skip_empties = skip_empties
    ...
    def validate(self, data):
        ...
        """
            AvoinHäme
            Alkuperäinen maksimimerkkimäärä short_description kentälle 160
        """
        errors = {}
        lang_error_msg = _('This field must be specified before an event is published.')
        for field in self.fields_needed_to_publish:
    ...
    def to_representation(self, obj):
        ret = super(EventPutDeleteSerializer, self).to_representation(obj)
...
class EventSerializer(LinkedEventsSerializer, GeoModelAPIView):
    ...
    class Meta:
        model = Event
        exclude = ['deleted', 'pin', 'provider_email']
        list_serializer_class = BulkListSerializer
...
def _filter_event_queryset(queryset, params, srs=None):
    ...
    if val:
        ...
        if keyword:
            raise ValidationError(_('Keyword search can\'t be used together with audience.'))

    # Tavastia Events
    # Filter by postal_code, multiple postal codes separated by comma
...
class EventViewSet(BulkModelViewSet, JSONAPIViewSet):
    ...
    def initial(self, request, *args, **kwargs):
        ...
        self.data_source, self.organization = get_data_source_and_publisher(request)
    ...
    """
        Tavastia Events
        Metodin tarkistus ja sen mukaan serializerin käyttäminen
        Poistettu serializer version tarkistaminen, ei pitäisi olla käyttöä uudella asennuksella
    """
    def get_serializer_class(self):
        serializer_class = self.serializer_class
        ...
        if self.request.method == 'POST':
            ...
            serializer_class = EventPutDeleteSerializer
        return serializer_class
    ...
    """
        Tavastia Events
        funktio perform_update poistettu käytöstä, korvattu pin koodin tarkistuksella
    """
    ...
    """
        Tavastia Events
        käyttöoikeuden tarkistus poistettu
    """
    ...
    """
    def perform_destroy(self, instance):
        if instance.pin != self.request.POST.get('pin', "default"):
            raise DRFPermissionDenied(_('Incorrect PIN.'))
        else:
            instance.soft_delete()

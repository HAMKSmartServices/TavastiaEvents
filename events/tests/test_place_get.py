...
"""
def place_create(idvalue):
    return Place.objects.create(
        id=idvalue,
        position=Point(50, 50),
        name_fi='Paikka 1',
        street_address_fi='Katuosoite 1',
        address_locality_fi='Kaupunki'
    )
...
def test_get_place_detail(api_client, place):
    ...
    place_create_results = place_create('123456789')
    response = get_detail(api_client, place_create_results.pk)
    assert response.data['id'] == place_create_results.id

...
def test__autopopulated_fields_at_create(
    ...
    assert event.created_by == None#user
    assert event.last_modified_by == None#user
    assert event.created_time is not None
    assert event.last_modified_time is not None
    #assert event.data_source.id == None#settings.SYSTEM_DATA_SOURCE_ID
    assert event.publisher == None#organization

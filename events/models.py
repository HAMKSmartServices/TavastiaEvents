...
class Image(models.Model):
    ...
    """
        Tavastia Events
        data_source blank=True lisätty
        license default arvo muutettu event_only
    """

    # Properties from schema.org/Thing
    name = models.CharField(verbose_name=_('Name'), max_length=255, db_index=True, default='')

    data_source = models.ForeignKey(DataSource, related_name='provided_%(class)s_data', db_index=True, null=True, blank=True)
    ...
    license = models.ForeignKey(License, verbose_name=_('License'), related_name='images', default='event_only')
    ...
    """
        Tavastia Events
        Admin puolelle nimen palautus
    """
    def __str__(self):
        name_to_admin = ""
        if self.name is not None:
            name_to_admin = self.name
        else:
            name_to_admin = "Ei nimeä"
        return name_to_admin
...
class BaseModel(models.Model):
    """
        Tavastia Events
        Data_source muutettu vapaaehtoiseksi
        funktiot is_user_editable ja is_user_edited poistettu
    """
    id = models.CharField(max_length=50, primary_key=True)
    data_source = models.ForeignKey(DataSource, related_name='provided_%(class)s_data', db_index=True, null=True, blank=True)
    ...
    """
        Tavastia Events
        Editointi oikeuksien tarkistus poistettu. Palvelussa ei käyttäjiä.
    """
    #def is_user_editable(self):
    #    return self.data_source.user_editable
...
class Place(MPTTModel, BaseModel, SchemalessFieldMixin):
    ...
    """
    publisher = models.ForeignKey('django_orghierarchy.Organization', verbose_name=_('Publisher'), db_index=True, null=True, blank=True)
...
class Event(MPTTModel, BaseModel, SchemalessFieldMixin):
    ...
    headline = models.CharField(verbose_name=_('Headline'), max_length=255, null=True, db_index=True, blank=True)
    ...
    publisher = models.ForeignKey('django_orghierarchy.Organization', verbose_name=_('Publisher'), db_index=True,
                                  on_delete=models.PROTECT, related_name='published_events', null=True, blank=True)
...
def keyword_added_or_removed(sender, model=None,
    ...
    if action in ('post_add', 'post_remove'):
        ...
        """
            Tavastia Events
            päivitetään keywordien ja paikkojen n_events kentät tapahtuman tallentamisen jälkeen
        """
        call_command('update_n_events')
